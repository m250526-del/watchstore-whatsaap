...
app.post('/api/send-verification', async (req, res) => {
  try {
    ...
    // Bilingual Message #1 (English + Urdu)
    const msg1Text =
      `Hello ${customerName || 'Customer'}!\n\n` +
      `We have received your order #${orderId}.\n` +
      `Order Total: Rs. ${amount || '0'}\n\n` +
      `Please confirm your order by pressing the button below.\n\n` +
      `------------------------------\n` +
      `السلام علیکم ${customerName || 'محترم'}!\n\n` +
      `ہمیں آپ کا آرڈر #${orderId} موصول ہو گیا ہے۔\n` +
      `آرڈر کی کل رقم: Rs. ${amount || '0'}\n\n` +
      `براہِ کرم نیچے دیا گیا بٹن دبا کر اپنے آرڈر کی تصدیق کریں۔`;

    const buttons = [
      {
        buttonId: `confirm_order_${orderId}`,
        buttonText: { displayText: 'Confirm Order / آرڈر کی تصدیق کریں' },
        type: 1,
      },
    ];
    ...
    try {
      ...
      console.warn('Button format fallback, sending text message:', btnErr);
...
async function startBaileys() {
  ...
  // Incoming Message Listener (Detect Confirm Order button & Screenshots)
  sock.ev.on('messages.upsert', async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      const isConfirmAction =
        ...
        textContent.toLowerCase().includes('confirm') ||
        textContent.includes('تصدیق') ||
        textContent === '1';

      if (isConfirmAction && pendingOrder.status === 'pending_confirmation') {
